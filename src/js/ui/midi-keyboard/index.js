...
module.exports = ({state, actions, params = {}}) => div('.midi-keyboard', params, [
	div('.body', [
		div('.keys', generateKeys('C4', 'B4').map(parseKey).map(key =>
			div(((key.isSharp || key.isFlat) ? '.black' : '.white'), {
				on: {
					mousedown: () => actions.arrToggle('pressedKeys', key.key),
					mouseup: () => actions.arrToggle('pressedKeys', key.key)
				},
				class: {
					on: state.pressedKeys.indexOf(key.key) > -1
				}
			},
